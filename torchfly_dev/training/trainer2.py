...
class Trainer2:
    ...
    def __setup_configuration(self):
        ...
        if self.config.training.validation_iterations_interval is None or \
            ...
            self.config.training.validation_iterations_interval = len(self.train_loader) // self.config.training.num_gpus_per_node - 1
    ...
    def validate(self):
        ...
        for batch in self.validation_loader:
            ...
            output = results['output'].argmax(dim=1)   # BS*10    
            target = batch["target"]  # BS
