...
def load_and_cache_examples(config, task, tokenizer, evaluate=False):
    ...
    if os.path.exists(cached_features_file) and not config.task.overwrite_cache:
        ...
        examples = (
            processor.get_dev_examples(config.task.data_dir) if evaluate else processor.get_train_examples(config.task.data_dir)
        )
...
def get_data_loader(config):
    ...
    train_sampler = RandomSampler(train_dataset) if config.training.num_gpus_per_node == 1 else DistributedSampler(train_dataset)
    train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=config.training.batch_size)
